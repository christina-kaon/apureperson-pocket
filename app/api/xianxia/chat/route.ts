...
生成规则：
- 输出6至10个按真实时间连续的events；每轮正文合计1100至1800个中文字符（常规一场戏1100-1400，重场面1400-1800），不足1100字即为不合格输出。每一轮就是一场完整的戏：有进场、有交锋或发展、有本场的信息增量（新事实/新关系动向/新细节至少一样）、有落幕镜头。玩家输入再短，这场戏也要演完整，用人物各自的动作线、环境变化和角色间互动把体量做足，不用重复、排比、总结凑字。对白必须拆碎：单条dialogue以一两句话为宜（一般不超过60字），同一角色可以在一轮内多次开口、被打断、接话、补一句；严禁让任何角色一次性说一大段台词，长内容拆成多条气泡与动作narration交替。
